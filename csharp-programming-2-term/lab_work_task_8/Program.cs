...
namespace lab_work_task_8
{
    class Program
    {
        ...
        public delegate void GiveInfoAboutPurchare(string message);
        static void Main(string[] args)
        {
            ...
            var morrreExtraIngridientPizza = new Pizza("morrreExtraIngridientPizza", 16, Pizza.TypePizza.Square, 22, 1500);
            /* add ingridients with another function */
            morrreExtraIngridientPizza.AddIngridients(AddAllExtraIngridientsToPizza);
            morrreExtraIngridientPizza.AddIngridients(AddAllExtraIngridientsToPizza);
            ...
            while (true)
            {
                ...
                switch (GetChoice(1, 9))
                {
                    ...
                    case 8:
                        ...
                        break;
                    case 9:
                        TryToCreateParty(foodStorage.Where(food => food is ICalories).ToList());
                        break;
                    default:
                        return;
                }
        ...
        static void Info()
        {
            ...
            Console.WriteLine("9. Try to create party with food");
            Console.WriteLine("10. Exit");
        }

        public static void TryToCreateParty(List <Food> foodWeNeed)
        {
            Random rand = new Random();
            try
            {
                Party party = new Party(rand.Next(-2, 7), 5000);
                for (int i = 0; i < 10; ++i)
                {
                    var tryToBuyItem = foodWeNeed[rand.Next(0, foodWeNeed.Count - 1)];
                    if (tryToBuyItem is Pizza)
                    {
                        Pizza tryToBuyItemPizza = (Pizza)tryToBuyItem;

                        /* lambda expression */

                        Pizza.GiveInfoAboutPurchare giveInfoAboutPurchare = (message) =>
                        {
                            if (message == "Successfully buying")
                            {
                                Console.WriteLine(String.Format("Pizza {0} with order {1} was successfully purchased", tryToBuyItemPizza.Name, tryToBuyItemPizza.NumberOrder));
                            }
                            else
                            {
                                Console.WriteLine(String.Format("Pizza {0} with order {1} was not purchased", tryToBuyItemPizza.Name, tryToBuyItemPizza.NumberOrder));
                            }
                        };

                        tryToBuyItemPizza.PizzaEvent += giveInfoAboutPurchare;

                        party.tryToBuyFood(tryToBuyItem);
                ...
                {
                    Console.WriteLine("Random is not allowed to start party(");
                }
            } 
            catch (Exception ex)
            {
        ...
        public static void AddAllExtraIngridientsToPizza(ref List<FastFood.AdditionalIngredients> a)
        {
            a.Add(FastFood.AdditionalIngredients.Cheese);
            a.Add(FastFood.AdditionalIngredients.Ketchup);
            a.Add(FastFood.AdditionalIngredients.Mayonnaise);
            a.Add(FastFood.AdditionalIngredients.Сutlet);
            a.Add(FastFood.AdditionalIngredients.Sausage);
        }
