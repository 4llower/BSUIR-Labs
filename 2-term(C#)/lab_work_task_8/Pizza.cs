...
namespace lab_work_task_8
{
    ...
    {
        public delegate void AddAllExtraIngridientsToPizza(ref List<AdditionalIngredients> additionals);

        public int NumberSlices { get; set; }

        public delegate void GiveInfoAboutPurchare(string message);

        public event GiveInfoAboutPurchare PizzaEvent;
        ...
        public void AddIngridients(AddAllExtraIngridientsToPizza fun)
        {
            fun(ref additionals);
        }

        public void AddMessageToEvent(string message)
        {
            PizzaEvent?.Invoke(message);
        }
